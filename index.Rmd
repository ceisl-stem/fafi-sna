...
programming language. This project 

### Network Graph Libraries

The igraph library [@csard2006igraph], [@thomas2023tidygraph], [@pedersen2022ggraph]
...
Blah AnthroTools blah [@purzycki2017anthrotools].
...
Use the [IBM Carbon Design System color pallete](https://carbondesignsystem.com/data-visualization/color-palettes/).

```{r constants, message = FALSE, warning = FALSE, echo = TRUE}
...
### Initial Network Graph Functions
...
calculate.centrality <- function(the.graph, the.salience, the.file) {
  ...
  data.laplacian <- laplacian(the.graph, mode = "in")
  data.leverage <- leverage(the.graph)
  data.infocent <- info.centrality.vertex(the.graph)
  data.leaderrank <- leaderrank(the.graph)
  data.constraint <- constraint(the.graph)
  data.katz <- katzcent(the.graph)
  data.latora <- closeness.latora(the.graph)
  data.bottleneck <- bottleneck(the.graph)
  analysis.network.data <- data.frame(indegree = data.in,
                                      outdegree = data.out,
                                      betweenness = data.betweenness,
                                      leaderrank = data.leaderrank,
                                      laplace = data.laplacian,
                                      leverage = data.leverage,
                                      infocent = data.infocent,
                                      constraint = data.constraint,
                                      latora = data.latora,
                                      bottleneck = data.bottleneck,
                                      katz = data.katz)
  #cor.test <- cor(analysis.network.data)
  #corrplot::corrplot(cor.test, method = "square", type = "lower")
...
calculate.keyactors <- function(the.frame, the.salience, the.file) {
  ...
  key.frame <- the.frame %>%
    #mutate(leverage = ((leverage - min_leverage) / (max_leverage - min_leverage))) |>
...
create.q.cent <- function(the.1, the.2, the.question) {
  the.q.cent <- bind_rows(the.1, the.2) |>
    ...
    select(question, actor, betweenness, outdegree, indegree, constraint, leverage,
           laplace, infocent, leaderrank, latora, SmithsS)
}
...
calculate.ranks <- function(the.cent) {
  the.cent <- the.cent |>
    ...
    mutate(infocent_rank = dense_rank(desc(infocent))) |>
    ...
    select(question, actor, outdegree, outdegree_rank, indegree, indegree_rank,
           ...
           leverage, leverage_rank, laplace, laplacian_rank, infocent, infocent_rank,
