...
![Family as Faculty as an Infrastructure](family_as_faculty.png)
...
***Save Plots Function.*** This function saves plots to the `output` folder in two formats,
...
***Create Correlation Plot Function.*** Correlation plots are effective modes of visualizing
...
***Calculate Tukey's Fences Function.*** Small graphs are prone to extreme outliers, especially
...
***Initialize Network Graph Function.*** The `set.graph` function takes in an edge list that
...
***Plot the Graph Function.*** The `draw.graph` creates the visualization--the plot--of the
...
***Calculate Centralities Function.*** A number of *node-level centralities,* or metrics,
...
***Create Rankings for Comparing Across Graphs.*** Because this is a multilayered graph
