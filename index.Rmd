...
preview: assets/fafi-sna-git-logo.png
author:
  - name: Jeremy F Price 
...
bibliography: assets/fafi-sna-references.bib
...
```{r project-identifier, include = FALSE}
...
![Family as Faculty as an Infrastructure Social Network Analysis Project](assets/fafi-sna-git-logo.png)
...
ggsave(full_plot, filename = "assets/sna_full-plot.png", width = 11.5, height = 8, units = "in", dpi = 300)
knitr::include_graphics("assets/sna_full-plot.png")
print(full_frame)
```
...
ggsave(grid_sna, filename = "assets/grid-sna.png", width = 11, height = 8.5, units = "in", dpi = 300)
knitr::include_graphics("assets/grid-sna.png")
```
...
# Save them as a high resolution image.
ggsave(grid_sna, filename = "assets/grid-key.png", width = 11, height = 8.5, units = "in", dpi = 300)
knitr::include_graphics("assets/grid-key.png")
```
...
<dfn>Ontological contexts</dfn> refer to structural and infrastructural
elements of social life.

<dfn>Phenomenological contexts</dfn> refer to personal experiential
considerations.
...
* Phenomenological
  ...
  - Learning Agendas [@ashDialogicInquiryLife2003]

Ontological and phenomenological contexts can inform one another, for example
`FAPE` and `Schooling Experiences` inform `Learning Agendas` but we consider
these separately.

### Consider Sociohistorical Context

```{r neighborhood-sparkline, echo=FALSE, results=FALSE, warning=FALSE, message=FALSE}
historic_census <- read_csv("data/historic_census.csv") |>
  filter(year != 1940)
historic_sparkline <- ggplot(
  historic_census,
  aes(
    x = year,
    y = population,
  ...
  geom_line() +
  theme_light() +
  labs(caption = "Chart: The Polis Center at IUPUI - Source: NHGIS.") +
  theme(
    legend.position = "top",
...
```{r unruly-timeline, echo = FALSE, results = TRUE, warning = FALSE, message=FALSE}
timeline_frame <- read_csv("data/timeline-data.csv",
  col_names = TRUE,
  show_col_types = FALSE
)
timeline_frame$layer <- factor(timeline_frame$layer,
  levels = c("outcome", "local", "sped", "national", "epoch")
)
timeline_plot <- gg_vistime(timeline_frame,
  ...
  # geom_segment(
  #   aes(
  #     x = as.POSIXct("1954-01-30"), y = 10,
  #     xend = as.POSIXct("1956-01-30"), yend = 34
  #   ),
  #   color = "#A7A9AB", linetype = 2, linewidth = 0.65
  # ) +
  # geom_segment(
  #   aes(
  #     x = as.POSIXct("1959-01-30"), y = 36,
  #     xend = as.POSIXct("1968-01-30"), yend = 35
  #   ),
  #   color = "#A7A9AB", linetype = 2, linewidth = 0.65
  # ) +
  # geom_segment(
  ...
  #     xend = as.POSIXct("1970-01-30"), yend = 35
  #   ),
  #   color = "#A7A9AB", linetype = 2, linewidth = 0.65
  # )
multilayered_chart <- ggarrange(historic_sparkline, timeline_plot, ncol = 1, nrow = 2, heights = c(2.5,6.5))
#plot_save(timeline_plot, "timeline")
```

```{r save-timeline, echo = FALSE, purl = FALSE, dpi=300, layout = "l-page"}
# Save a copy of the network to be displayed below
ggsave(multilayered_chart,
  filename = "assets/timeline-plot.png",
  width = 18,
  height = 10,
  units = "in",
  dpi = 300
...
ggsave(multilayered_chart,
  filename = "assets/timeline-plot.pdf",
  width = 18,
  height = 10,
  units = "in",
  dpi = 300
)
knitr::include_graphics("assets/timeline-plot.png")
```
...
Appreciation to Harish Gadde and Jay Colbert of the [Polis Center at IUPUI](https://polis.iupui.edu/)
for providing the historical census data.

## Author Contributions {.appendix}

```{r author-contributions, purl=FALSE}
...
author_contributions |>
  ...
  tab_source_note(
    source_note = md("Available in [JATS](assets/credit.xml) format.")
  )
```
